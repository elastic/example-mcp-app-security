 ...
 *
 * Dispatches over the entity `type` (`alert | host | user | process | ip`)
 * and shapes per-type ES searches into a uniform {@link EntityDetail}
 * surface. Failures inside a single sub-search are swallowed (returning an
 * empty list) so a single index outage degrades a card rather than failing
 * the whole call.
 */
export class EntityDetailService {
  ...
  private async getAlertDetail(ruleName: string): Promise<EntityDetail> {
    ...
    const threats = src["kibana.alert.rule.threat"] as
      | Array<{
          tactic?: { name?: string };
    ...
    return {
      ...
      fields: [
        ...
        {
          label: "Risk Score",
          value: String(src["kibana.alert.risk_score"] || ""),
        },
        {
          label: "Status",
          value: String(src["kibana.alert.workflow_status"] || ""),
        },
        { label: "Reason", value: String(src["kibana.alert.reason"] || "") },
        {
          label: "Host",
          value: String(
            (src.host as Record<string, unknown>)?.name || "N/A"
          ),
          mono: true,
        },
        {
          label: "User",
          value: String(
            (src.user as Record<string, unknown>)?.name || "N/A"
          ),
          mono: true,
        },
        {
          label: "Process",
          value: String(
            (src.process as Record<string, unknown>)?.name || "N/A"
          ),
          mono: true,
        },
        {
          label: "Executable",
          value: String(
            (src.process as Record<string, unknown>)?.executable || "N/A"
          ),
          mono: true,
        },
        ...
        ...(src["kibana.alert.rule.description"]
          ? [
              {
                label: "Description",
                value: String(src["kibana.alert.rule.description"]),
              },
            ]
          : []),
      ],
  ...
  private async getHostDetail(hostName: string): Promise<EntityDetail> {
    ...
    fields.push({
      ...
      value: netR
        .map((n) => `${n["destination.ip"]}:${n["destination.port"]}`)
        .filter(Boolean)
        .join(", "),
      mono: true,
    });
    ...
    if (alertR.length > 0) {
      fields.push({
        ...
        value: alertR
          .map(
            (a) =>
              `${a["kibana.alert.severity"]} — ${a["kibana.alert.rule.name"]}`
          )
          .join("\n"),
      });
    }
    ...
    const events = procR.map((p) => ({
      ...
      action: "process",
      detail: `${p["user.name"] || "?"} ran ${p["process.name"]}${
        p["process.command_line"]
          ? ": " + String(p["process.command_line"]).substring(0, 80)
          : ""
      }`,
    }));
  ...
  private async getUserDetail(userName: string): Promise<EntityDetail> {
    ...
    const hosts = [
      ...new Set(procR.map((p) => p["host.name"]).filter(Boolean)),
    ];
    ...
    if (alertR.length > 0) {
      fields.push({
        ...
        value: alertR
          .map(
            (a) =>
              `${a["kibana.alert.severity"]} — ${a["kibana.alert.rule.name"]} (${a["host.name"]})`
          )
          .join("\n"),
      });
    }
  ...
  private async getProcessDetail(processName: string): Promise<EntityDetail> {
    const procR = await this.safeSearch(
      "logs-endpoint.events.process-*",
    ...
    const fields: EntityDetail["fields"] = [
      ...
      {
        label: "Executable",
        value: String(first["process.executable"] || "N/A"),
        mono: true,
      },
      {
        label: "Parent",
        value: String(first["process.parent.name"] || "N/A"),
        mono: true,
      },
      {
        label: "Parent Executable",
        value: String(first["process.parent.executable"] || "N/A"),
        mono: true,
      },
      {
        label: "Hosts",
      ...
      },
      {
        label: "Users",
        value: [
          ...new Set(procR.map((p) => p["user.name"]).filter(Boolean)),
        ].join(", "),
        mono: true,
      },
    ];
    ...
    if (first["process.command_line"]) {
      fields.push({
        label: "Command Line",
        value: String(first["process.command_line"]),
        mono: true,
      });
    }
  ...
  private async getIPDetail(ip: string): Promise<EntityDetail> {
    const netR = await this.safeSearch(
      "logs-endpoint.events.network-*",
      { "destination.ip": ip },
      5,
      [
    ...
    const fields: EntityDetail["fields"] = [
      { label: "IP Address", value: ip, mono: true },
      {
        ...
        value: [
          ...new Set(netR.map((n) => n["host.name"]).filter(Boolean)),
        ].join(", "),
        mono: true,
      },
      {
        label: "Processes",
        value: [
          ...new Set(netR.map((n) => n["process.name"]).filter(Boolean)),
        ].join(", "),
        mono: true,
      },
      {
        label: "Ports",
        value: [
          ...new Set(netR.map((n) => n["destination.port"]).filter(Boolean)),
        ].join(", "),
        mono: true,
      },
    ];
    ...
    const events = netR.map((n) => ({
      ...
      action: "connection",
      detail: `${n["process.name"]} → ${ip}:${n["destination.port"]} (${
        n["network.protocol"] || "?"
      })`,
    }));
